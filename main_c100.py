...
from utils.c100_dataloader import *
...
parser.add_argument('--milestone_3', default=10, type=int)
...
for args.model in models_:
    if args.model == 'res_26':
        ...
        net = net.cuda()
        ckpt = torch.load('ckpt' + '/ckpt_c100_bn.pth')
        ...
        for args.level in severity:
            ...
            for args.corruption in common_corruptions:
                ...
                te_loader, _ = prepare_test_data(args)
                ...
                for epoch in range(1, args.nepoch + 1):
                    ...
                    if args.save_model:
                        if err_cls <= min(err_corr):
                            ...
                            Path(f"c100_adapted_models/{args.corruption}/").mkdir(parents=True,
                                                                                 exist_ok=True)
                            torch.save(state, f"c100_adapted_models/" + str(args.corruption) + '.pth')
            ...
            Path(f'results/cifar100/{args.model}/').mkdir(exist_ok=True, parents=True)
            np.save(f'results/cifar100/{args.model}/{args.level}.npy', all_res)

    if args.model == 'augmix':
        print(f'Backbone ::: {args.model}')
        all_res = []
        net = WideResNet(widen_factor=2, num_classes=100, depth=40)

        net = net.cuda()
        ckpt = torch.load('ckpt/augmix_c100.pt')
        ...
        for args.level in severity:
            ...
            for args.corruption in common_corruptions:
                ...
                scheduler = torch.optim.lr_scheduler.MultiStepLR(
                    optimizer, [args.milestone_1, args.milestone_2], gamma=0.5, last_epoch=-1)
                ...
                for epoch in range(1, args.nepoch + 1):
                    ...
                    if args.save_model:
                        if err_cls <= min(err_corr):
                            ...
                            Path("aug_mix_c_100_ckpt/").mkdir(parents=True, exist_ok=True)
                            torch.save(state,
                                       'aug_mix_c_100_ckpt/' + str(args.corruption) + '.pth')
                min_err_corr = min(err_corr)
                print(f'Minimum Error after adaptation: {min_err_corr: .1f}')
                all_res.append(min_err_corr)
            print('Mean Error: ', sum(all_res) / len(all_res))
